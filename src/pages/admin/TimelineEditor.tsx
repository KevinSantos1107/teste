...
const emptyEvent = (): Partial<TimelineEvent> => ({ title: '', date: '', description: '', location: '', isSecret: false, side: 'left' });
...
export default function TimelineEditor() {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      ...
      setEvents(loaded);
    } catch { show('Erro ao carregar eventos', 'err'); }
    setLoading(false);
  }, [siteId]);
  ...
  const openEdit = (event: TimelineEvent) => {
    ...
    setPhotoPreview(event.photoUrl || (event.publicId ? cloudinaryUrl(event.publicId, { w: 400 }) : null));
  ...
  const handleSave = async () => {
    if (!form.title || !form.date || !form.description) { show('Título, data e descrição são obrigatórios', 'err'); return; }
    setSaving(true);
    try {
  ...
  const EventForm = (
    <div className="bg-slate-800 border border-rose-500/40 rounded-xl p-6 space-y-4">
      <h2 className="font-bold text-white text-lg">{editingId === 'new' ? 'Criar Novo Evento' : 'Editar Evento'}</h2>
      <div className="grid md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-300">Título *</label>
          <Input value={form.title || ''} onChange={e => setForm({ ...form, title: e.target.value })} className="bg-slate-900 border-slate-700 text-slate-200" placeholder="Ex: Primeiro Beijo" />
        </div>
        <div className="space-y-2">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input value={form.location || ''} onChange={e => setForm({ ...form, location: e.target.value })} className="bg-slate-900 border-slate-700 text-slate-200" placeholder="Ex: Paris, França" />
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium text-slate-300">Lado na Timeline</label>
          <select value={form.side || 'left'} onChange={e => setForm({ ...form, side: e.target.value as 'left' | 'right' })}
      ...
      {/* Photo upload */}
      <div className="space-y-2">
        <label className="text-sm font-medium text-slate-300">Foto {form.publicId || form.photoUrl ? '(clique para trocar)' : '(opcional)'}</label>
        {photoPreview ? (
          <div className="flex items-center gap-3">
            <img src={photoPreview} alt="" className="w-24 h-24 rounded-xl object-cover border border-slate-600" />
            <div className="flex flex-col gap-2">
              <button onClick={() => fileRef.current?.click()} className="flex items-center gap-1.5 text-sm text-slate-300 hover:text-white border border-slate-600 rounded-lg px-3 py-1.5 transition-colors">
                <Upload className="w-4 h-4" /> Trocar foto
              </button>
              <button onClick={() => { setPhotoFile(null); setPhotoPreview(null); setForm({ ...form, publicId: undefined, photoUrl: undefined }); }} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1">
      ...
      <label className="flex items-center gap-3 cursor-pointer">
        <div
          ...
          className={cn('w-11 h-6 rounded-full transition-colors relative', form.isSecret ? 'bg-rose-500' : 'bg-slate-700')}
        >
          <div className={cn('absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform', form.isSecret ? 'translate-x-5' : 'translate-x-0')} />
        </div>
        <span className="text-sm text-slate-300">Evento secreto (não aparece na timeline pública)</span>
      </label>
      <div className="flex gap-3 pt-2">
        <Button onClick={handleSave} isLoading={saving} className="gap-2"><CheckCircle className="w-4 h-4" /> Salvar Evento</Button>
        <Button variant="secondary" onClick={() => setEditingId(null)} className="bg-slate-700 text-white">Cancelar</Button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button onClick={openNew} className="gap-2 shrink-0"><Plus className="w-4 h-4" /> Novo Evento</Button>
      </div>
                ...
                {editingId !== event.id && (
                  ...
                  >
                    ...
                    <div className="w-16 h-16 rounded overflow-hidden bg-slate-800 flex-shrink-0 border border-slate-700/50">
                      {event.publicId ? (
                        <img src={cloudinaryUrl(event.publicId, { w: 100, h: 100, c: 'fill' })} alt="" className="w-full h-full object-cover" />
                      ) : event.photoUrl ? (
                        <img src={event.photoUrl} alt="" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full flex items-center justify-center text-slate-600"><Calendar className="w-5 h-5"/></div>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <h3 className="font-bold text-white">{event.title}</h3>
                        <span className="text-xs font-mono bg-slate-700 text-slate-400 px-2 py-0.5 rounded">{event.date}</span>
                        {event.isSecret && <span className="text-xs bg-rose-500/20 text-rose-400 px-2 py-0.5 rounded">secreto</span>}
                      </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0 opacity-50 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(event)} className="p-2 text-slate-400 hover:text-white rounded-lg hover:bg-slate-700 transition-colors"><Edit2 className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(event)} className="p-2 text-slate-400 hover:text-red-400 rounded-lg hover:bg-slate-700 transition-colors"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                )}
              </div>
