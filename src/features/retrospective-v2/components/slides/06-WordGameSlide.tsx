...
export function WordGameSlide({ onNext }: WordGameSlideProps) {
  ...
  const WORD = (config.wordGameAnswer || 'INCRIVEL').toUpperCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^A-Z]/g, '');
  ...
  for (let i = 0; i < MAX_GUESSES; i++) {
    if (i < guesses.length) {
      ...
      const cells: LetterCell[] = Array(WORD_LENGTH).fill(null).map((_, j) => ({
    ...
    >
      ...
      <div className="z-10 w-full flex flex-col items-center px-2 pt-16 pb-4 gap-3 max-h-full h-full relative">
        ...
        <div className="text-center w-full px-2 z-10">
          <p className="text-white/50 text-[10px] uppercase tracking-widest mb-1">Capítulo 5 · Jogo</p>
          ...
          {rows.map((row, ri) => (
            ...
            >
              {row.cells.map((cell, ci) => {
                ...
                return (
                  <div key={ci} className="relative perspective-1000" style={{ width: WORD_LENGTH > 6 ? '36px' : '44px', height: WORD_LENGTH > 6 ? '36px' : '44px' }}>
                    <div 
                      className={cn(
            ...
            <div className="flex flex-col items-center text-center max-w-sm w-full">
              {won ? (
                <>
                  ...
                  <p className="text-rose-400 font-bold text-base mb-8 italic text-balance">🎀 Sim, você acertou!</p>
                  
                  <button onClick={e => { e.stopPropagation(); onNext(); }} className="w-full py-3 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-full shadow-[0_0_20px_rgba(243,24,96,0.4)] transition-all active:scale-95 flex items-center justify-center gap-2 text-sm uppercase tracking-wider">
                ...
                <>
                  ...
                  <p className="text-white/80 font-bold mb-6">A palavra era: <span className="text-rose-400 uppercase">{WORD}</span></p>
                  <p className="text-white/60 text-sm mb-8 italic">Tente novamente para prosseguir!</p>
                  
                  <button onClick={e => { e.stopPropagation(); restart(); }} className="w-full py-3 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-full shadow-[0_0_20px_rgba(243,24,96,0.4)] transition-all active:scale-95 flex items-center justify-center gap-2 text-sm uppercase tracking-wider">
