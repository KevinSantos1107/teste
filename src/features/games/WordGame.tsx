...
function loadStats(): GameStats {
  try {
    return (
      JSON.parse(localStorage.getItem('wg_stats_v2') || 'null') ?? {
        gamesPlayed: 0,
        wins: 0,
        currentStreak: 0,
        bestStreak: 0,
    ...
    return { gamesPlayed: 0, wins: 0, currentStreak: 0, bestStreak: 0 };
  }
}
function saveStats(s: GameStats) {
  localStorage.setItem('wg_stats_v2', JSON.stringify(s));
}
function loadUsedWords(): string[] {
  try {
    return JSON.parse(localStorage.getItem('wg_used_words') || '[]');
  } catch {
    return [];
  }
}
function saveUsedWords(u: string[]) {
  localStorage.setItem('wg_used_words', JSON.stringify(u));
}
  ...
  const startNewRound = useCallback((qs: Question[], used: string[]) => {
    ...
    qs.forEach((q) =>
      q.palavras.forEach((p) => {
        ...
        if (w.length === 5) {
          // double check length 5 constraint
          const id = `${q.id}:${w}`;
  ...
  const handleKey = useCallback(
    (key: string) => {
      ...
      if (key === 'ENTER') {
        ...
        const newRow: LetterCell[] = currentInput
          .split('')
            ...
            {row.cells.map((cell, ci) => {
              ...
              return (
                <div
                  ...
                  style={{
                    width: wordLength > 6 ? '36px' : '44px',
                    height: wordLength > 6 ? '36px' : '44px',
                  }}
                >
                  <div
                    className={cn(
            ...
            {won ? (
              <>
                ...
                <div className="flex w-full gap-3">
                  <button
                    onClick={() => startNewRound(questions, usedWords.current)}
                    className="flex-1 py-3 bg-rose-500 hover:bg-rose-600 text-white font-bold rounded-full shadow-[0_0_20px_rgba(243,24,96,0.4)] transition-all active:scale-95 flex items-center justify-center gap-2 text-sm uppercase tracking-wider"
                  >
                    <Play className="w-4 h-4 fill-current" /> Próxima
                  </button>
                  <button
                    onClick={shareOnWhatsApp}
                    className="flex-1 py-3 bg-emerald-500 hover:bg-emerald-600 text-white font-bold rounded-full shadow-[0_0_20px_rgba(16,185,129,0.4)] transition-all active:scale-95 flex items-center justify-center gap-2 text-sm uppercase tracking-wider"
                  >
              ...
              <>
                ...
                <p className="text-white/80 font-bold mb-6">
                  A palavra era: <span className="text-rose-400 uppercase">{currentWord}</span>
                </p>
